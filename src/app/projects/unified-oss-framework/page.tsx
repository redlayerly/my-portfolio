...
export default function UnifiedOSSPage() {
  const { t } = useLang()
    ...
    <div className="min-h-screen" style={{ background: '#0a0a0f', color: '#e4e4e7' }}>
      <section className="relative h-[60vh] flex items-center justify-center overflow-hidden">
        <div className="absolute inset-0">
          <div className="absolute inset-0 bg-cover bg-center" style={{ backgroundImage: 'url(/images/projects/unified-oss.png)' }} />
        ...
        <div className="relative z-10 text-center px-6 max-w-4xl">
          ...
          <h1 className="text-4xl md:text-6xl font-bold mb-4">{t.unifiedOss.title}</h1>
          <p className="text-lg text-gray-400">{t.unifiedOss.desc}</p>
        </div>
      </section>
        ...
        <div className="space-y-12">
          <div>
            ...
            <p className="text-gray-300 leading-relaxed text-lg">
              {t.unifiedOss.abstractText}
            </p>
          </div>

          <div>
            <h2 className="text-2xl font-bold mb-4" style={{ color: '#00f0ff' }}>{t.projectCommon.novelContributions}</h2>
            <div className="grid md:grid-cols-2 gap-6">
              {t.unifiedOss.contributions.map((item, i) => (
          ...
          <div className="flex gap-4">
            <a href="https://github.com/Chihi-Sahati/Unified-OSS-Framework" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 px-6 py-3 rounded-lg font-bold text-sm uppercase tracking-wider text-black" style={{ background: 'linear-gradient(135deg, #00f0ff, #00c8d4)' }}>
      ...
      <section className="border-t py-8 px-6" style={{ borderColor: 'rgba(255,255,255,0.05)' }}>
        <div className="max-w-5xl mx-auto text-center text-gray-500 text-sm">
          <p><strong className="text-gray-400">{t.projectCommon.author}:</strong> Al-Hussein A. Al-Sahati | <strong className="text-gray-400">{t.projectCommon.supervisor}:</strong> Dr. Houda Chihi</p>
        </div>
      </section>
    </div>
  )
